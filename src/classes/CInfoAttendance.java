...
public class CInfoAttendance
{
    ...
    public AttendanceState GetAttendanceState()
    {
        ...
        if (LastEntry.State == AttendanceState.TIME_OUT && new Date().before(LastEntry.Date)) // Let the user timein when the last entry is a timeout and a day has passed
        ...
        return AttendanceState.TIME_IN; // Cant timein or timeout if the user has filled the quota for today
    }
